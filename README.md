...
- Title / Body の全文AI品質評価（Gemini API）
- 問題一覧を `.xlsx` で生成
- ブラウザからレポートをダウンロード
...
- TypeScript
- Tailwind CSS
- Gemini API（Structured Output + Zod検証）
...
- `GEMINI_API_KEY`（必須）
- `GEMINI_MODEL`（任意、未指定時は `gemini-2.5-flash-lite`）
...
1. Google AI StudioでAPIキーを発行
2. `GEMINI_API_KEY` に設定
3. 必要なら `GEMINI_MODEL` を変更

本アプリは `models/*:generateContent` を利用し、`responseMimeType=application/json` + `responseJsonSchema` で構造化JSONを受け取ります。

## デフォルトモデル方針

- デフォルト: `gemini-2.5-flash-lite`
- 理由: 2026-03-31時点で低コスト帯かつStructured Output用途に使いやすいため
- 必要に応じて `GEMINI_MODEL` で他モデルへ切替可能
